...
def enrichDay(symbol, day):
    ...
    results = su.es_search("symbols-1d", query)['hits']['hits']
    data = {}
    for d in results:
        doc = d['_source']
        data[d['_id']] = doc
    ...
    su.es_bulk_create("symbols-1d-aug", data, partial=500, es="ml-demo" )
...
def get_augmentation_period(symbol: str):
    ...
    if su.es.indices.exists( f"symbols-1d-aug"):
        start_aug, end_aug = query_first_and_last_doc( symbol, "symbols-1d-aug", "ml-demo")
        if not end_aug:
            day = start_1d
        else:
            day = end_aug
